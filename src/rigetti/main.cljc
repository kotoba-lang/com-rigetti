(ns rigetti.main
  "Kotodama WASM entrypoint for the Rigetti clean-room actor (L4) — Clojure port.
  ...
  facts (`rigetti.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rigetti")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rigetti.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rigetti-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rigetti-compat/ready)
